...
// canvas
var svg = d3.select(document.getElementById('linegraph')).append("svg")
...
d3.json("data.json", function(error, linedata) {
...
function D3Map () {
  ...
  // fill dataset in appropriate format
    dataset.forEach(function(item){ //
        ...
        // console.log(dataset) 
        mapdata[iso] = { languages: lang, fillColor: paletteScale(value) };
        // console.log(mapdata)
    });
  ...
  geographyConfig: {
    ...
    popupTemplate: function(geo, mapdata) {
      // Don't show if no mapdata present
      if (!mapdata) { return ; }
      // else return the mapdata
        return ['<div class="hoverinfo"><strong>',
                'Languages in ' + geo.properties.name,
                ': ' + mapdata.languages.split(",").length, '<br>', mapdata.languages.split(","),
                '</strong></div>'].join('');
        }
